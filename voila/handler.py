...
class VoilaHandler(ExtensionHandler):
    ...
    def get(self, path=None):
        ...
        for html_snippet, resources in self.exporter.generate_from_notebook_node(
    ...
    def _jinja_notebook_execute(self, nb, kernel_id):
        ...
        result.cells = list(
            filter(
                lambda cell: filter_empty_code_cells(cell, self.exporter), result.cells
            )
        )
    ...
    def fix_notebook(self, notebook):
        ...
        all_kernel_specs = yield tornado.gen.maybe_future(
        ...
        if kernel_name not in all_kernel_specs:
            ...
            kernel_name = yield self.find_kernel_name_for_language(
    ...
    def find_kernel_name_for_language(self, kernel_language, kernel_specs=None):
        ...
        if kernel_language in self.config.language_kernel_mapping:
            raise tornado.gen.Return(
                self.config.language_kernel_mapping[kernel_language]
            )  # TODO py2: replace by return
        if kernel_specs is None:
            kernel_specs = yield tornado.gen.maybe_future(
                self.kernel_spec_manager.get_all_specs()
            )
        matches = [
            name
