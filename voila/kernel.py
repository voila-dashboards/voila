...
class KernelWarmer(object):
    '''Kernel warmer instantiates and executes notebooks via the KernelExecutor class'''
    def __init__(self,
                 notebook_path,
                 contents_manager,
                 config_manager,
        ...
        self.traitlet_config = traitlet_config
...
class KernelExecutor(object):
    '''Kernel Executor executes a notebook'''
    def __init__(self,
                 notebook_path,
                 contents_manager,
                 config_manager,
    ...
    async def preload(self, base_url, path=None, request=None):
        ...
        if not self.notebook:
            raise tornado.web.HTTPError(500, 'Notebook could not be determined')
        ...
        if request:
            self.kernel_env['SCRIPT_NAME'] = request.request.path
            self.kernel_env['QUERY_STRING'] = str(request.request.query)
            self.kernel_env['SERVER_PROTOCOL'] = str(request.request.version)
            host, port = split_host_and_port(request.request.host.lower())
            self.kernel_env['SERVER_PORT'] = str(port) if port else ''
            self.kernel_env['SERVER_NAME'] = host
        else:
            self.kernel_env['SCRIPT_NAME'] = notebook_path
            self.kernel_env['QUERY_STRING'] = ''
            self.kernel_env['SERVER_PROTOCOL'] = ''
            self.kernel_env['SERVER_PORT'] = ''
            self.kernel_env['SERVER_NAME'] = ''

        self.kernel_env['PATH_INFO'] = ''
        ...
        if 'voila' in self.notebook.metadata and self.voila_configuration.allow_template_override in ['YES', 'NOTEBOOK']:
            template_override = self.notebook.metadata['voila'].get('template')
        if self.voila_configuration.allow_template_override == 'YES' and request:
            template_override = request.get_argument("voila-template", template_override)
        ...
        if 'voila' in self.notebook.metadata and self.voila_configuration.allow_theme_override in ['YES', 'NOTEBOOK']:
            theme = self.notebook.metadata['voila'].get('theme', theme)
        if self.voila_configuration.allow_theme_override == 'YES' and request:
            theme = request.get_argument("voila-theme", theme)
